...
def pop_per_year():
    ...
    pop_per_year = pop_per_year[['Country Name', 'Country Code'] + [str(i) for i in range(1971, 2019)]]
    country_continent = county_continent()
    pop_per_year = pop_per_year.join(country_continent.set_index('ISO (3)'), on='Country Code')
    pop_per_year = pop_per_year.groupby('Continent').sum().T.reset_index()
    pop_per_year = pop_per_year[(pop_per_year['index'] != 'Note') & (pop_per_year['index'] != 'ISO (No)')]
    return deepcopy(pop_per_year)
...
def main():
    ...
    pop = pop_per_year()
    for c, color in zip(continent_list, colors):
        fig.add_trace(go.Scatter(x=pop['index'], y=pop[c],
                                 name=c,
                                 mode='lines',
                                 legendgroup="group",
                                 line_color=color
