    ...
    for i in range(5):
        for j in range(3):
            ...
            fig.add_trace(go.Bar(
                ...
                marker_color=get_power_plant_color(power_plant_types[i * 3 + j])
            ...
            #     name=f'Cumulative Sum {power_plant_types[i*3 + j]}'
            # ), secondary_y=False, row=i+1, col=j+1)

            fig.update_xaxes(range=[1896, 2018], row=i + 1, col=j + 1)

    fig.update_layout(title_text="New Power Plants per type in the period 1896-2018")
